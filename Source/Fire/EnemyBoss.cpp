...
float AEnemyBoss::TakeDamage(float DamageCount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (Health > 0)
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 3, FColor::Silver, FString::Printf(TEXT("Hit")));

		if (Health / MaxHealth <= 0.25f * Phase)
		{
			GetWorld()->GetTimerManager().SetTimer(TCompleteStun, this, &ThisClass::CompleteStun, 3.f, false);
